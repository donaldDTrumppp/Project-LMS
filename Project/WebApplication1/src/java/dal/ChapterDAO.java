...
//        return listChapter;
//    }
    public ArrayList<Chapter> getChapterBySubjectID(int id, int start, int total)
            throws SQLException, ClassNotFoundException {
        ArrayList<Chapter> listChapter = new ArrayList<Chapter>();
        DBContext connect = new DBContext();
        Connection con = null;
    ...
    public int getNoOfRecords(int id) throws SQLException, ClassNotFoundException {
        ...
        try {
            //1. Connect DB
            //2. Create SQL String
            String sql = "SELECT COUNT(*) FROM chapter WHERE subject_id = ?";
            //3. Create Statement
            stm = DBContext().prepareStatement(sql);
            stm.setInt(1, id);
            //4. Execute Query
    ...
    public boolean addNewChapter(Chapter chapter) {
        ...
        AccountDAL ad = new AccountDAL();
        try {
            String sql = "INSERT INTO chapter (chapter_name, created_at, created_by, description, display_order, subject_id, status) VALUES (?, ?, ?, ?, ?, ?, ?)";
            PreparedStatement st = DBContext().prepareStatement(sql);
            st.setString(1, chapter.getChapterName());
            st.setTimestamp(2, chapter.getCreatedAt());
            int createrID = chapter.getCreatedBy().getID();
            ...
            st.setInt(5, chapter.getDisplayOrder());
            int subjectID = sd.GetSubjectID(chapter.getSubject());
            st.setInt(6, subjectID);
            st.setInt(7, chapter.getStatus());
